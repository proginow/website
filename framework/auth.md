...
# Auth
[Source Code On Github](https://github.com/proginow/auth/)
...
// $db = \Proginow\Db\PdoDatabase::fromDsn(new \Proginow\Db\PdoDsn('mysql:dbname=my-database;host=localhost;charset=utf8mb4', 'my-username', 'my-password'));
// or
// $db = \Proginow\Db\PdoDatabase::fromDsn(new \Proginow\Db\PdoDsn('pgsql:dbname=my-database;host=localhost;port=5432', 'my-username', 'my-password'));
// or
// $db = \Proginow\Db\PdoDatabase::fromDsn(new \Proginow\Db\PdoDsn('sqlite:../Databases/my-database.sqlite'));

$auth = new \Proginow\Auth\Auth($db);
```
...
catch (\Proginow\Auth\InvalidEmailException $e) {
    die('Invalid email address');
}
catch (\Proginow\Auth\InvalidPasswordException $e) {
    die('Invalid password');
}
catch (\Proginow\Auth\UserAlreadyExistsException $e) {
    die('User already exists');
}
catch (\Proginow\Auth\TooManyRequestsException $e) {
...
catch (\Proginow\Auth\EmailNotVerifiedException $e) {
...
catch (\Proginow\Auth\InvalidSelectorTokenPairException $e) {
    die('Invalid token');
}
catch (\Proginow\Auth\TokenExpiredException $e) {
    die('Token expired');
}
catch (\Proginow\Auth\UserAlreadyExistsException $e) {
...
catch (\Proginow\Auth\ResetDisabledException $e) {
...
catch (\Proginow\Auth\NotLoggedInException $e) {
    die('Not logged in');
}
catch (\Proginow\Auth\InvalidPasswordException $e) {
...
catch (\Proginow\Auth\ConfirmationRequestNotFound $e) {
...
Here’s how to use this library with your own tables for custom user information in a maintainable and re-usable way:
 ...
 1. If you need the custom user information only rarely, you may just retrieve it as needed. If you need it more frequently, however, you’d probably want to have it in your session data. The following method is how you can load and access your data in a reliable way:
    ...
    ```php
    function getUserInfo(\Proginow\Auth\Auth $auth) {
        if (!$auth->isLoggedIn()) {
            return null;
        }
...
```php
if ($auth->hasRole(\Proginow\Auth\Role::SUPER_MODERATOR)) {
    echo 'The user is a super moderator';
}

// or

if ($auth->hasAnyRole(\Proginow\Auth\Role::DEVELOPER, \Proginow\Auth\Role::MANAGER)) {
    echo 'The user is either a developer, or a manager, or both';
}

// or

if ($auth->hasAllRoles(\Proginow\Auth\Role::DEVELOPER, \Proginow\Auth\Role::MANAGER)) {
...
```php
\Proginow\Auth\Role::ADMIN;
\Proginow\Auth\Role::AUTHOR;
\Proginow\Auth\Role::COLLABORATOR;
\Proginow\Auth\Role::CONSULTANT;
\Proginow\Auth\Role::CONSUMER;
\Proginow\Auth\Role::CONTRIBUTOR;
\Proginow\Auth\Role::COORDINATOR;
\Proginow\Auth\Role::CREATOR;
\Proginow\Auth\Role::DEVELOPER;
\Proginow\Auth\Role::DIRECTOR;
\Proginow\Auth\Role::EDITOR;
\Proginow\Auth\Role::EMPLOYEE;
\Proginow\Auth\Role::MAINTAINER;
\Proginow\Auth\Role::MANAGER;
\Proginow\Auth\Role::MODERATOR;
\Proginow\Auth\Role::PUBLISHER;
\Proginow\Auth\Role::REVIEWER;
\Proginow\Auth\Role::SUBSCRIBER;
\Proginow\Auth\Role::SUPER_ADMIN;
\Proginow\Auth\Role::SUPER_EDITOR;
\Proginow\Auth\Role::SUPER_MODERATOR;
\Proginow\Auth\Role::TRANSLATOR;
```
...
```php
\Proginow\Auth\Role::getMap();
// or
\Proginow\Auth\Role::getNames();
// or
\Proginow\Auth\Role::getValues();
```
...
```php
function canEditArticle(\Proginow\Auth\Auth $auth) {
...
final class MyRole {
    ...
    const CUSTOMER_SERVICE_AGENT = \Proginow\Auth\Role::REVIEWER;
    const FINANCIAL_DIRECTOR = \Proginow\Auth\Role::COORDINATOR;
...
catch (\Proginow\Auth\UnknownIdException $e) {
    die('Unknown ID');
}
```
...
catch (\Proginow\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
catch (\Proginow\Auth\AmbiguousUsernameException $e) {
...
```php
try {
    $auth->admin()->addRoleForUserById($userId, \Proginow\Auth\Role::ADMIN);
}
catch (\Proginow\Auth\UnknownIdException $e) {
    die('Unknown user ID');
}
...
try {
    $auth->admin()->addRoleForUserByEmail($userEmail, \Proginow\Auth\Role::ADMIN);
}
catch (\Proginow\Auth\InvalidEmailException $e) {
    die('Unknown email address');
}
...
try {
    $auth->admin()->addRoleForUserByUsername($username, \Proginow\Auth\Role::ADMIN);
}
catch (\Proginow\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
...
```php
try {
    $auth->admin()->removeRoleForUserById($userId, \Proginow\Auth\Role::ADMIN);
}
catch (\Proginow\Auth\UnknownIdException $e) {
    die('Unknown user ID');
}
...
try {
    $auth->admin()->removeRoleForUserByEmail($userEmail, \Proginow\Auth\Role::ADMIN);
}
catch (\Proginow\Auth\InvalidEmailException $e) {
    die('Unknown email address');
}
...
try {
    $auth->admin()->removeRoleForUserByUsername($username, \Proginow\Auth\Role::ADMIN);
}
catch (\Proginow\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
...
```php
try {
    if ($auth->admin()->doesUserHaveRole($userId, \Proginow\Auth\Role::ADMIN)) {
...
```php
\Proginow\Auth\Auth::createRememberCookieName();
```
...
```php
$length = 24;
$randomStr = \Proginow\Auth\Auth::createRandomString($length);
```

#### Creating a UUID v4 as per RFC 4122

```php
$uuid = \Proginow\Auth\Auth::createUuid();
```

### Reading and writing session data

For detailed information on how to read and write session data conveniently, please refer to [the documentation of the session library](https://github.com/proginow/cookie#reading-and-writing-session-data), which is included by default.
